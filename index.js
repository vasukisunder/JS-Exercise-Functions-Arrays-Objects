...
*/
function getName(testObject) {
  const ans = "Hello, my name is " + testObject.name;
  return ans;
}
...
function getCarInfoByIndex(inventory, index) {
  ...
  const ans = inventory[index].car_make + " " + inventory[index].car_model;
  
  
  return "This is a " + ans
}
...
function getLastCarInfo(inventory) {
  const ans = inventory[inventory.length-1].car_make + " " + inventory[inventory.length-1].car_model;
...
function getCarInfoById(inventory, id) {
  const ans = inventory[id-1].car_make + " " + inventory[id-1].car_model;
...
function sortCarInventory(inventory) {
  ...
  const ans = inventory.sort(function(a, b){
    var nameA=a.car_model.toLowerCase(), nameB=b.car_model.toLowerCase()
...
function getModelYears(inventory) {
  function getModel(inventory, field) {
    var output = [];
  for (var i=0; i < inventory.length; i++)
    output.push(inventory[i][field]);
    return output;
}

const ans = getModel(inventory, "car_year");
...
function carMaker(num) {
  const ans = {
    odometer: num,
    drive: function(dist){
      ans.odometer = ans.odometer + dist;
      return ans.odometer;
    }
  }
  return ans;
